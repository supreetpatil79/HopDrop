...
export const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(function TextArea(
  { label, error, helperText, labelHint, containerClassName, className, id, required, disabled, ...props },
  ref
) {
  ...
  return (
    <label className={clsx('flex w-full flex-col gap-2', containerClassName)}>
      ...
      <span
        className={clsx(
          'group rounded-2xl border border-border/80 bg-white px-4 shadow-[0_10px_28px_-24px_rgba(15,23,42,0.65)] transition duration-200',
          error
      ...
      >
        <textarea
          ...
          className={clsx(
            'min-h-[120px] w-full resize-y border-0 bg-transparent px-0 py-3 text-[15px] text-text outline-none placeholder:text-slate-400 focus:outline-none focus:ring-0 disabled:cursor-not-allowed disabled:text-slate-400',
            className
          )}
          {...props}
          required={required}
