...
export default function RegisterPage() {
  ...
  const sendOtpMutation = useMutation({
    ...
    onSuccess: () => {
      trackFunnelStep('carrier_activation', 'signup_otp_requested');
      toast.success('OTP sent successfully');
    },
    onError: (error) => {
      captureClientError(error, { source: 'carrier_register_send_otp' });
    }
  });
  ...
  const registerMutation = useMutation({
    ...
    onError: (error) => {
      captureClientError(error, { source: 'carrier_register_submit' });
    }
  });
